...
//    READ();
    string x, y, z;
    cin >> x >> y >> z;
    vector<pair<int,int>>a(3, make_pair(0, 0));
    a[x[0] - 'A'].first += x[1] == '>' ? 1 : -1;
    a[x[2] - 'A'].first += x[1] == '>' ? -1 : 1;

    a[y[0] - 'A'].first += y[1] == '>' ? 1 : -1;
    a[y[2] - 'A'].first += y[1] == '>' ? -1 : 1;

    a[z[0] - 'A'].first += z[1] == '>' ? 1 : -1;
    a[z[2] - 'A'].first += z[1] == '>' ? -1 : 1;
    a[0].second = 0;
    a[1].second = 1;
    a[2].second = 2;
    sort(a.begin(), a.end());
    set<int>s = {a[0].first, a[1].first, a[2].first};
    if (s.size() != 3) cout << "Impossible\n";
    else {
      for(auto el : a) {
        cout << (char) (el.second + 'A');
      }
    }
